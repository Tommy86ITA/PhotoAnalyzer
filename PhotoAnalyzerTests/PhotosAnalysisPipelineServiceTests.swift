...
struct PhotosAnalysisPipelineServiceTests {
    ...
    @Test func photosSourceFacadeRunsPickerItemsThroughFilePipeline() async throws {
        let outputURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: outputURL, withIntermediateDirectories: true)
        defer {
        ...
        let dependencies = AnalysisPipelineDependencies(
            ...
            buildStatistics: { _ in statistics },
            exportDataFiles: { _, _, _, _, paths, _, _ in paths },
            exportContactSheet: { _, _, _, _, _ in },
            archivePackage: { paths, _ in paths.archiveURL }
        )
        let service = PhotosSourceAnalysisService(
            materializePickerItems: { _ in
                let workspace = try TemporaryAssetWorkspace(rootDirectoryURL: outputURL)
                let fileURL = workspace.fileURL(preferredFilename: "picker.jpg", fallbackBasename: "picker")
                _ = FileManager.default.createFile(atPath: fileURL.path, contents: Data())
                let asset = MaterializedPhotosAsset(
                    assetLocalIdentifier: "picker-item-1",
                    originalFilename: "picker.jpg",
                    fileURL: fileURL,
                    representation: .current
                )
                return PhotosMaterializationResult(assets: [asset], skippedAssets: [], workspace: workspace)
            },
            pipelineService: AnalysisPipelineService(dependencies: dependencies)
        )

        let result = try await service.run(
            input: .pickerItems([], datasetName: "Picker"),
            outputFolderURL: nil,
            progressHandler: nil
        )
